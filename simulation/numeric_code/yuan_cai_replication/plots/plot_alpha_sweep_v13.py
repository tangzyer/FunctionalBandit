"""Plot Lepski coverage/width for v13 (separate kappa for Aniso/Iso).

Kappa = C * sqrt(log(L) + log(n)), with C_aniso and C_iso potentially different.
3 methods: Aniso-theory, Iso-theory, FPCA.
"""
...
if __name__ == '__main__':
    ...
    for C_a, C_i in C_pairs:
        ...
        fig.suptitle(rf'v13: Lepski ($C_A={C_a}$, $C_I={C_i}$) + Adapt.Trunc (10-fold CV, $\mu_n / (\log n)^2$)',
                     fontsize=12, y=1.0)
        plt.tight_layout(rect=[0, 0.08, 1, 0.97])
        out_name = f'inference_alpha_sweep_v13_7methods_Ca{C_a}_Ci{C_i}.pdf'
