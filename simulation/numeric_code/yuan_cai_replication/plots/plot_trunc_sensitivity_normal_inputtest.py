"""Truncation-level sensitivity figure, NORMAL slope b_k ~ k^{-2} (the
D1-FAILS regime), INPUT-distribution test functions. Same format as the beta4
truncation-sensitivity figure: 2x3 grid with rows = covariance design (Aligned,
Haar) and cols = r_2 in {0.5, 1, 2}. Within each panel, six series:
Trunc.Aniso and Adapt.Trunc, each at q in {0.2, 0.3, 0.4}.

Method encoded by marker shape, q encoded by a print-safe colour.

Writes three PDFs (companion to the beta4 sensitivity figure):
  inference_trunc_sensitivity_normal_legend_inputtest.pdf
  inference_trunc_sensitivity_normal_coverage_inputtest.pdf
  inference_trunc_sensitivity_normal_width_inputtest.pdf
"""
...
def render_panel(ax, panel_data, *, quantity, conf_levels, x_positions,
    ...
    if quantity == 'cov':
        ...
        lo_lim, hi_lim = max(0, y_lo - pad), min(1.01, y_hi + pad)
    else:
        y_lo, y_hi = min(series_lo), max(series_hi)
        pad = (y_hi - y_lo) * 0.06
        lo_lim, hi_lim = max(0.0, y_lo - pad), y_hi + pad
    # Do NOT set ylim here: with sharey='row' the last panel would win and clip
    # the others. render_grid_figure aggregates these limits across the row.
...
def render_grid_figure(all_panels, *, quantity, r2_info, design_info,
    ...
    for ri, (design_key, design_label) in enumerate(design_info):
        ...
        for ci, (r2_val, r2_label) in enumerate(r2_info):
            ...
            lo, hi = render_panel(ax, panel_data, quantity=quantity,
        ...
        # sharey='row': set one range covering every column so no marker in the
        # row is clipped (the q=0.2 coverage crash at small r_2 reaches ~0).
        axes[ri, 0].set_ylim(min(row_lo), max(row_hi))
...
def _design_case(design_key, r2_val):
    """Map (design, r2) -> normal-slope (b_k ~ k^{-2}) case_name."""
    if design_key == 'aligned':
        return f'aligned_r2_{_r2_tag(r2_val)}'
    if design_key == 'haar':
        return f'haar_r2_{_r2_tag(r2_val)}'
