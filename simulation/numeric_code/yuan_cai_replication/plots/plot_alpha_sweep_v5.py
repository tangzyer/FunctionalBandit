"""Plot alpha sweep: aniso lam/(logn)^3, iso lam/logn, FPCA sqrt(n)."""
...
if __name__ == '__main__':
    ...
    # Method indices: 1 = aniso lam/(logn)^3, 3 = iso lam/logn, 4 = FPCA
    ...
    for case_name, _ in case_info:
        ...
        for n_val in n_values:
            fname = f'alpha_sweep_{case_name}_n{n_val}_v2.npz'
    ...
    fig.suptitle(r'Coverage & CI Width, $r_2=2$, Aniso $\lambda/(\log n)^3$ + Iso $\lambda/\log n$, 1000 test fns',
                 fontsize=13, y=1.0)
    plt.tight_layout(rect=[0, 0.04, 1, 0.97])
    plt.savefig(os.path.join(results_dir, 'inference_alpha_sweep_v5.pdf'),
