"""Plot one-sided coverage + two-sided CI width for Haar r2=2.

3 rows: left coverage, right coverage, CI width.
Lambda grid 10^-5 to 1, excess-risk tuned.
"""
...
if __name__ == '__main__':
    ...
    for n_val in n_values:
        fname = f'alpha_sweep_haar_r2_2_n{n_val}_v6.npz'
    ...
    fig.suptitle(r'Haar ($r_2=2$), $\lambda$ grid $[10^{-5}, 1]$, excess-risk tuned, 1000 test fns',
                 fontsize=12, y=1.0)
    plt.tight_layout(rect=[0, 0, 1, 0.98])
    plt.savefig(os.path.join(results_dir, 'inference_alpha_sweep_v8_haar.pdf'),
