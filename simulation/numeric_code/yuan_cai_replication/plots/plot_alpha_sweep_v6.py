"""Plot one-sided coverage (left & right) for aniso lam/(logn)^3, iso lam/log(logn), FPCA."""
...
def load_and_compute_onesided(npz_path, n_val, target_conf_levels=(0.75, 0.85, 0.95)):
    ...
    # left_cov[m, si, t] = fraction of datasets where true_val <= y_hat + q * se
    # right_cov[m, si, t] = fraction of datasets where true_val >= y_hat - q * se
    ...
    for m in range(n_methods):
        for si, ai in enumerate(indices):
            ...
            if m == 4:  # FPCA uses t-distribution
                J = int(np.sqrt(n_val))
                df = n_val - J
                q = t_dist.ppf(1 - alpha, df)  # one-sided quantile
            else:
                q = norm.ppf(1 - alpha)  # one-sided quantile

            upper = all_yhat[m] + q * all_se[m]  # (n_datasets, n_test)
            lower = all_yhat[m] - q * all_se[m]

            # Left: (-inf, upper), covers if true_val <= upper
            left_covered = (true_vals[None, :] <= upper)
            left_cov[m, si, :] = left_covered.mean(axis=0)

            # Right: (lower, inf), covers if true_val >= lower
...
if __name__ == '__main__':
    ...
    # Method indices: 1 = aniso lam/(logn)^3, 3 = iso lam/log(logn), 4 = FPCA
    ...
    for case_name, _ in case_info:
        ...
        for n_val in n_values:
            fname = f'alpha_sweep_{case_name}_n{n_val}_v3.npz'
    ...
    # 2 rows (left, right) x 3 columns (cases)
    ...
    fig.suptitle(r'One-sided Coverage, $r_2=2$, Aniso $\lambda/(\log n)^3$ + Iso $\lambda/\log\log n$, 1000 test fns',
                 fontsize=13, y=1.0)
    plt.tight_layout(rect=[0, 0.04, 1, 0.97])
    plt.savefig(os.path.join(results_dir, 'inference_alpha_sweep_v6_onesided.pdf'),
