"""Plot Lepski coverage/width with small kappas: 0.01, 0.03, 0.1, 0.3.

Data from v8 files (9 methods: 4 aniso + 4 iso + FPCA).
Generates two plots: aniso kappa comparison, iso kappa comparison.
"""
...
if __name__ == '__main__':
    ...
    colors_a = ['tab:blue', 'tab:cyan', 'tab:orange', 'tab:red', 'tab:purple']
    markers_a = ['o', 'p', 's', '^', 'v']
    n_ma = len(method_indices_a)
    jitter_a = np.linspace(-0.3, 0.3, n_ma)
    ...
    fig.suptitle(r'Aniso Lepski: small $\kappa$ comparison, 1000 test fns',
                 fontsize=13, y=1.0)
    plt.tight_layout(rect=[0, 0.04, 1, 0.97])
    plt.savefig(os.path.join(results_dir, 'inference_alpha_sweep_v42_aniso_kappa.pdf'),
                bbox_inches='tight', dpi=150)
    plt.close()
    print('Saved plot v42 (Aniso kappa comparison)')

    # --- Plot 2: Iso kappa comparison ---
    method_indices_i = [4, 5, 6, 7, 8]
    method_names_i = [
        r'Iso $\kappa=0.01$',
        r'Iso $\kappa=0.03$',
        r'Iso $\kappa=0.1$',
    ...
    colors_i = ['tab:blue', 'tab:cyan', 'tab:green', 'tab:red', 'tab:purple']
    markers_i = ['o', 'p', 'D', '^', 'v']
    n_mi = len(method_indices_i)
    jitter_i = np.linspace(-0.3, 0.3, n_mi)
    ...
    fig2.legend(handles2, labels2, loc='lower center', ncol=6, fontsize=9,
                bbox_to_anchor=(0.5, -0.02))
    fig2.suptitle(r'Iso Lepski: small $\kappa$ comparison, 1000 test fns',
                  fontsize=13, y=1.0)
    plt.tight_layout(rect=[0, 0.04, 1, 0.97])
    plt.savefig(os.path.join(results_dir, 'inference_alpha_sweep_v42_iso_kappa.pdf'),
                bbox_inches='tight', dpi=150)
    plt.close()
    print('Saved plot v42 (Iso kappa comparison)')
