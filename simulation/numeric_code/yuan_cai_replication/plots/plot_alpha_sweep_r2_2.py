"""Plot alpha sweep for r2=2: 3 confidence levels x 3 sample sizes, jittered error bars."""
...
if __name__ == '__main__':
    ...
    for case_name, _ in case_info:
        ...
        for n_val in n_values:
            if case_name == 'shifted':
                if n_val == 256:
                    fname = 'alpha_sweep_shifted.npz'
                else:
                    fname = f'alpha_sweep_shifted_n{n_val}.npz'
            else:
        ...
        for row, (metric_name, ylabel) in enumerate([
            ...
            for mi, m_data_idx in enumerate(method_indices):
                ...
                for n_val in n_values:
                    ...
                    pt = cov_pt if metric_name == 'coverage' else width_pt
                    for ci in range(n_conf):
            ...
            row_label = 'CI Width' if row == 1 else 'Coverage'
    ...
    fig.suptitle(r'Coverage & CI Width at 75%, 85%, 95% Confidence, $n \in \{64, 256, 1024\}$, $r_2=2$',
                 fontsize=14, y=1.0)
    plt.tight_layout(rect=[0, 0.04, 1, 0.97])
    plt.savefig(os.path.join(results_dir, 'inference_alpha_sweep_r2_2.pdf'),
                bbox_inches='tight', dpi=150)
    plt.close()
    print('Saved r2=2 plot')
