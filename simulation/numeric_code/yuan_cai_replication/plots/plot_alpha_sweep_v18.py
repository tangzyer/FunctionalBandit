...
Aniso: (logn)^4 loglogn, Iso: logn loglogn.
Uses v6 data (excess-risk oracle lambda, grid 10^-12 to 1).
"""
...
if __name__ == '__main__':
    ...
    method_names = [
        r'Aniso $\lambda/((\log n)^4\log\log n)$',
        r'Iso $\lambda/(\log n\,\log\log n)$',
        r'FPCA $\sqrt{n}$',
    ]
    ...
    fig.suptitle(r'Aniso $\lambda/((\log n)^4\log\log n)$, Iso $\lambda/(\log n\,\log\log n)$, grid $[10^{-12},1]$, 1000 test fns',
                 fontsize=13, y=1.0)
    plt.tight_layout(rect=[0, 0.04, 1, 0.97])
    plt.savefig(os.path.join(results_dir, 'inference_alpha_sweep_v18_3cases.pdf'),
