"""Plot Lepski coverage/width: bootstrap kappa vs theoretical kappa.

Data from v11 files (5 methods: Aniso-boot, Aniso-theory, Iso-boot, Iso-theory, FPCA).
"""
...
def load_and_compute(npz_path, n_val, target_conf_levels=(0.75, 0.85, 0.95)):
    ...
    kappa_theory = float(data['kappa_theory'])
    kappa_boot_aniso_mean = float(data['kappa_boot_aniso'].mean())
    kappa_boot_iso_mean = float(data['kappa_boot_iso'].mean())

    return two_sided_cov, width_per_test, kappa_theory, kappa_boot_aniso_mean, kappa_boot_iso_mean
...
if __name__ == '__main__':
    ...
    for case_name, _ in case_info:
        ...
        for n_val in n_values:
            ...
            cov, wid, kt, kba, kbi = load_and_compute(path, n_val)
            all_data[case_name][n_val] = (cov, wid)
            kappa_info[case_name][n_val] = (kt, kba, kbi)
    ...
    # --- Plot 1: Aniso (bootstrap vs theoretical) ---
    # m=0: Aniso bootstrap, m=1: Aniso theoretical, m=4: FPCA
    method_indices_a = [0, 1, 4]
    colors_a = ['tab:blue', 'tab:orange', 'tab:purple']
    markers_a = ['o', 's', 'v']
    n_ma = len(method_indices_a)
    jitter_a = np.linspace(-0.2, 0.2, n_ma)
        ...
        for row, (metric, row_title, ylabel) in enumerate(row_info):
            ...
            for mi, m_idx in enumerate(method_indices_a):
                ...
                for n_val in n_values:
                    ...
                    kt, kba, _ = kappa_info[case_name][n_val]
                    for ci in range(n_conf):
                ...
                # Build label for first panel only
                if col == 0 and row == 0:
                    if m_idx == 0:
                        label = r'Aniso (bootstrap $\kappa$)'
                    elif m_idx == 1:
                        label = r'Aniso (theory $\kappa$)'
                    else:
                        label = r'FPCA $\sqrt{n}$'
                else:
                    label = None
            ...
            for n_val in n_values:
                ...
                kappa_text = (f'$n={n_val}$\n'
                              f'boot $\\kappa$={kba:.2f}\n'
                              f'theory $\\kappa$={kt:.2f}')
                ax.text(center, -0.15, kappa_text,
    ...
    fig.suptitle(r'Aniso Lepski: bootstrap $\kappa$ vs theoretical $\kappa$',
                 fontsize=13, y=1.0)
    plt.tight_layout(rect=[0, 0.06, 1, 0.97])
    plt.savefig(os.path.join(results_dir, 'inference_alpha_sweep_v45_aniso.pdf'),
                bbox_inches='tight', dpi=150)
    plt.close()
    print('Saved plot v45 (Aniso)')

    # --- Plot 2: Iso (bootstrap vs theoretical) ---
    # m=2: Iso bootstrap, m=3: Iso theoretical, m=4: FPCA
    method_indices_i = [2, 3, 4]
    colors_i = ['tab:blue', 'tab:green', 'tab:purple']
    markers_i = ['o', 'D', 'v']
    n_mi = len(method_indices_i)
    jitter_i = np.linspace(-0.2, 0.2, n_mi)
        ...
        for row, (metric, row_title, ylabel) in enumerate(row_info):
            ...
            for mi, m_idx in enumerate(method_indices_i):
                ...
                for n_val in n_values:
                    two_sided_cov, width_pt = all_data[case_name][n_val]
                    pt = two_sided_cov if metric == 'coverage' else width_pt
                    kt, _, kbi = kappa_info[case_name][n_val]
                    for ci in range(n_conf):
                ...
                if col == 0 and row == 0:
                    if m_idx == 2:
                        label = r'Iso (bootstrap $\kappa$)'
                    elif m_idx == 3:
                        label = r'Iso (theory $\kappa$)'
                    else:
                        label = r'FPCA $\sqrt{n}$'
                else:
                    label = None
    ...
    fig2.legend(handles2, labels2, loc='lower center', ncol=4, fontsize=9,
                bbox_to_anchor=(0.5, -0.02))
    fig2.suptitle(r'Iso Lepski: bootstrap $\kappa$ vs theoretical $\kappa$',
                  fontsize=13, y=1.0)
    plt.tight_layout(rect=[0, 0.06, 1, 0.97])
    plt.savefig(os.path.join(results_dir, 'inference_alpha_sweep_v45_iso.pdf'),
                bbox_inches='tight', dpi=150)
    plt.close()
    print('Saved plot v45 (Iso)')
