"""Plot Lepski coverage/width with very small kappas: 0.001, 0.003, 0.01.

Data from v9 files (7 methods: 3 aniso + 3 iso + FPCA).
"""
...
if __name__ == '__main__':
    ...
    for case_name, _ in case_info:
        ...
        for n_val in n_values:
            fname = f'alpha_sweep_{case_name}_n{n_val}_v9.npz'
    ...
    fig.suptitle(r'Aniso Lepski: very small $\kappa$, 1000 test fns',
                 fontsize=13, y=1.0)
    plt.tight_layout(rect=[0, 0.04, 1, 0.97])
    plt.savefig(os.path.join(results_dir, 'inference_alpha_sweep_v43_aniso_kappa.pdf'),
                bbox_inches='tight', dpi=150)
    plt.close()
    print('Saved plot v43 (Aniso)')
    ...
    fig2.legend(handles2, labels2, loc='lower center', ncol=5, fontsize=9,
                bbox_to_anchor=(0.5, -0.02))
    fig2.suptitle(r'Iso Lepski: very small $\kappa$, 1000 test fns',
                  fontsize=13, y=1.0)
    plt.tight_layout(rect=[0, 0.04, 1, 0.97])
    plt.savefig(os.path.join(results_dir, 'inference_alpha_sweep_v43_iso_kappa.pdf'),
                bbox_inches='tight', dpi=150)
    plt.close()
    print('Saved plot v43 (Iso)')
