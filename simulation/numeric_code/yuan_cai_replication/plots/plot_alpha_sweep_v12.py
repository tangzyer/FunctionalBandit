"""Plot Lepski coverage/width for multiple c values.

Data from v12 files (3 methods: Aniso-theory, Iso-theory, FPCA).
One plot per c value.
"""
...
def load_and_compute(npz_path, n_val, target_conf_levels=(0.75, 0.85, 0.95)):
    ...
    return two_sided_cov, width_per_test, kappa_theory
...
if __name__ == '__main__':
    ...
    c_values = [0.001, 0.003, 0.0003]

    method_names = [
        r'Aniso (theory $\kappa$)',
        r'Iso (theory $\kappa$)',
        r'FPCA $\sqrt{n}$',
    ]
    ...
    for c_val in c_values:
        ...
        for case_name, _ in case_info:
            ...
            for n_val in n_values:
                fname = f'alpha_sweep_{case_name}_n{n_val}_c{c_val}_v12.npz'
                path = os.path.join(results_dir, fname)
                cov, wid, kt = load_and_compute(path, n_val)
                all_data[case_name][n_val] = (cov, wid)
                kappa_info[case_name][n_val] = kt
        ...
        for col, (case_name, case_label) in enumerate(case_info):
            for row, (metric, row_title, ylabel) in enumerate(row_info):
                ...
                for n_val in n_values:
                    ...
                    kt = kappa_info[case_name][n_val]
                    kappa_text = f'$n={n_val}$\n$\\kappa$={kt:.2f}'
                    ax.text(center, -0.13, kappa_text,
        ...
        fig.suptitle(rf'v12: Lepski — $\kappa=1.1\,z_{{\gamma_n/2}}$, $\gamma_n=n^{{-{c_val}}}$, grid $(2^{{-20}}, 1/n)$',
                     fontsize=12, y=1.0)
        plt.tight_layout(rect=[0, 0.06, 1, 0.97])
        out_name = f'inference_alpha_sweep_v12_c{c_val}.pdf'
