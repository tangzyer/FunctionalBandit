"""Plot two-sided coverage + CI width for predrisk oracle lambda.

2 rows (two-sided coverage, width) x 3 columns (cases).
5 methods: Aniso oracle, Aniso divided, Iso oracle, Iso divided, FPCA.
Uses v4 data (predrisk oracle lambda).
"""
...
if __name__ == '__main__':
    ...
    method_indices = [0, 1, 2, 3, 4]
    method_names = [
        r'Aniso oracle $\lambda$',
        r'Aniso $\lambda_{\mathrm{oracle}} / (\log n)^3$',
        r'Iso oracle $\lambda$',
        r'Iso $\lambda_{\mathrm{oracle}} / \log\log n$',
        r'FPCA $\sqrt{n}$',
    ]
    colors = ['tab:orange', 'tab:red', 'tab:green', 'tab:blue', 'tab:purple']
    markers = ['s', '^', 'D', 'v', 'o']
    ...
    fig.suptitle(r'Prediction-risk oracle $\lambda$ (per test fn), 1000 test fns',
                 fontsize=13, y=1.0)
    plt.tight_layout(rect=[0, 0.05, 1, 0.97])
    plt.savefig(os.path.join(results_dir, 'inference_alpha_sweep_predrisk_3cases.pdf'),
