"""Plot Lepski coverage/width for v13 — r2=1.5 cases only (Aligned + Haar)."""
...
def load_and_compute(npz_path, n_val, target_conf_levels=(0.75, 0.85, 0.95)):
    ...
    for m in range(n_methods):
        for si, ai in enumerate(indices):
            ...
            elif m in (3, 5):  # TruncAniso/AdaptTrunc n^0.3
                J = min(int(n_val ** 0.3), 50)
                df = n_val - J
                q_two = t_dist.ppf(1 - alpha / 2, df)
            elif m in (4, 6):  # TruncAniso/AdaptTrunc n^0.4
                J = min(int(n_val ** 0.4), 50)
                df = n_val - J
                q_two = t_dist.ppf(1 - alpha / 2, df)
            else:
...
if __name__ == '__main__':
    ...
    case_info = [
        ('aligned_r2_1p5', r'Aligned ($r_2=1.5$)'),
        ('haar_r2_1p5', r'Haar ($r_2=1.5$)'),
    ]
    ...
    jitter = np.linspace(-0.3, 0.3, n_m)
    ...
    for C_a, C_i in C_pairs:
        ...
        fig, axes = plt.subplots(2, n_cols, figsize=(12, 9))
        ...
        fig.suptitle(rf'v13: $r_2=1.5$ — Lepski ($C_A={C_a}$, $C_I={C_i}$) + Adapt.Trunc (10-fold CV, $\mu_n / (\log n)^2$)',
                     fontsize=12, y=1.0)
        plt.tight_layout(rect=[0, 0.08, 1, 0.97])
        out_name = f'inference_alpha_sweep_v13_r2_1p5_7methods_Ca{C_a}_Ci{C_i}.pdf'
