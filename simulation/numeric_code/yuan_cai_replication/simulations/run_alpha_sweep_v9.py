"""Alpha sweep with fixed lambda = 1/(n*(log n)^6).

Supports aligned_r2_2, haar_r2_2, shifted cases.
All aniso/iso methods use lambda = 1/(n*(log n)^6).
...
Usage: python run_alpha_sweep_v9.py <case_name> <n_val>
...
def run(case_name, n_val, results_dir):
    ...
    n_datasets = 1000
    n_test = 1000
    lam_inf = 1.0 / (n_val * np.log(n_val)**6)
    ...
    print(f'  lambda = 1/(n*(log n)^6) = {lam_inf:.6e} for n={n_val}')
